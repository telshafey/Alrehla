        ...
        <div className="bg-white p-3 rounded border mb-2 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-2 bg-red-50 text-red-700 rounded text-xs border border-red-100 opacity-60">
                    <span className="block font-bold mb-1">القيمة الحالية:</span>
                    <ValueFormatter value={oldValue} />
                </div>
                <div className="p-2 bg-green-50 text-green-700 rounded text-xs border border-green-200">
                    <span className="block font-bold mb-1">تعديل المقترح (متاح للمدير):</span>
                    {typeof newValue === 'string' && newValue.length > 50 ? (
                        ...
                        <div className="bg-white/50 p-2 rounded">
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1">تعديل المصفوفات يتم عبر الملف الشخصي المباشر.</p>
                        </div>
                    ) : (
                        <Input 
...
const AdminInstructorDetailPage: React.FC = () => {
    ...
    // Edited values for pending updates
    ...
    const handleEditedUpdateChange = (key: string, value: any, subKey?: string) => {
        setEditedUpdates((prev: any) => {
            const newState = { ...prev };
            if (subKey) {
                newState[key] = { ...newState[key], [subKey]: value };
            } else {
                newState[key] = value;
            }
            return newState;
    ...
    const renderPendingProfileUpdates = () => {
        if (!instructor || !editedUpdates) return null;
        
        return (
            <div className="space-y-2">
                {Object.keys(editedUpdates).map(key => {
                         ...
                         return Object.keys(editedUpdates[key]).map(itemId => {
                            ...
                            return (
                                <ChangeEditor 
                                    key={`${key}-${itemId}`} 
                                    label={`سعر: ${itemName}`} 
                                    oldValue={(instructor as any)[key]?.[itemId]} 
                                    newValue={editedUpdates[key][itemId]}
                                    onEdit={(val) => handleEditedUpdateChange(key, val, itemId)}
                                />
                            );
                        });
                    }
                    return (
                        <ChangeEditor 
                            ...
                            oldValue={(instructor as any)[key]} 
                            newValue={editedUpdates[key]} 
                            onEdit={(val) => handleEditedUpdateChange(key, val)}
                        />
                    );
                })}
            </div>
                    ...
                    {instructor.profile_update_status === 'pending' && (
                        <Card className="border-2 border-orange-400 shadow-lg bg-orange-50/10">
                            <CardHeader className="bg-orange-400/10 border-b border-orange-200">
                                <CardTitle className="flex items-center gap-2 text-orange-700"><FileEdit /> مراجعة واعتماد البيانات</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-6">
                                <p className="text-xs text-orange-600 mb-4 bg-orange-100 p-2 rounded">يمكنك تعديل القيم المقترحة بالأسفل قبل الضغط على "اعتماد" ليتم حفظ النسخة التي تراها مناسبة.</p>
                                {renderPendingProfileUpdates()}
                                <div className="p-3 bg-muted/50 rounded-lg mt-4 text-xs">
                                    <p className="font-bold mb-1">مبرر طلب المدرب:</p>
                                    <p>{instructor.pending_profile_data?.justification || 'لا يوجد مبرر مكتوب'}</p>
                                    <p className="mt-2 text-muted-foreground italic">تاريخ الطلب: {formatDate(instructor.pending_profile_data?.requested_at)}</p>
                                </div>
                                <div className="flex gap-3 mt-6">
                                    <Button 
                                        variant="success" 
                                        className="flex-1" 
                                        onClick={() => approveInstructorProfileUpdate.mutate({instructorId: instructor.id, modifiedUpdates: editedUpdates})} 
                                        loading={approveInstructorProfileUpdate.isPending} 
                                        icon={<Check />}
                                    >
                                        اعتماد التعديلات
                                    </Button>
                    ...
                    {instructor.schedule_status === 'pending' && (
                         <Card className="border-2 border-blue-400 shadow-lg bg-blue-50/10">
                            <CardHeader className="bg-blue-400/10 border-b border-blue-200">
                                <CardTitle className="flex items-center gap-2 text-blue-700"><Clock /> مراجعة تحديث الجدول</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-6">
                                <div className="space-y-3">
                                    {Object.entries((instructor.pending_profile_data?.proposed_schedule as WeeklySchedule) || {}).map(([day, times]) => (
                                        <div key={day} className="flex justify-between border-b pb-1 text-sm">
                                            <span className="font-bold">{dayNames[day]}</span>
                                            <span className="text-blue-700 font-mono">{times.join(', ')}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="flex gap-3 mt-6">
