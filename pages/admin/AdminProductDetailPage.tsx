...
// FIX: Corrected import path for useAdminPersonalizedProducts hook after refactoring.
...
import type { PersonalizedProduct, ImageSlotConfig, TextFieldConfig } from '../../lib/database.types';
...
const AdminProductDetailPage: React.FC = () => {
    ...
    return (
         <div className="animate-fadeIn space-y-8">
            <Link to="/admin/personalized-products" className="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-800 font-semibold mb-4">
                <ArrowLeft size={16} />
            ...
            <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <AdminSection title="المعلومات الأساسية" icon={<></>}>
                         ...
                         <AdminSection title="التسعير" icon={<></>}>
                        ...
                        <AdminSection title="حقول النصوص المخصصة" icon={<></>}>
                            <div className="space-y-4">
                                {(product.text_fields || []).map((field, index) => (
                                    <div key={field.id || index} className="p-4 border rounded-lg bg-gray-50 grid grid-cols-2 gap-4 items-end">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <label className="flex items-center gap-2"><input type="checkbox" checked={field.required} onChange={(e) => handleDynamicListChange('text_fields', index, 'required', e.target.checked)} /> إلزامي</label>
                                            <Button type="button" variant="danger" size="icon" onClick={() => removeDynamicListItem('text_fields', index)}><Trash2 size={16}/></Button>
                                        </div>
                                    </div>
                                ))}
                         ...
                         <AdminSection title="حقول رفع الصور" icon={<></>}>
                            <div className="space-y-4">
                                 {(product.image_slots || []).map((slot, index) => (
                                    <div key={slot.id || index} className="p-4 border rounded-lg bg-gray-50 grid grid-cols-2 gap-4 items-center">
                                         ...
                                         <label className="flex items-center gap-2"><input type="checkbox" checked={slot.required} onChange={(e) => handleDynamicListChange('image_slots', index, 'required', e.target.checked)} /> إلزامي</label>
                                         <Button type="button" variant="danger" size="icon" onClick={() => removeDynamicListItem('image_slots', index)} className="justify-self-end"><Trash2 size={16}/></Button>
                                    </div>
                                ))}
                        ...
                        <AdminSection title="إدارة الأهداف المحددة مسبقاً" icon={<></>}>
                            <div className="space-y-4">
                                {(product.story_goals || []).map((goal, index) => (
                                    <div key={goal.key || index} className="p-4 border rounded-lg bg-gray-50 grid grid-cols-2 gap-4 items-center">
                                        ...
                                        <div className="col-span-2 flex justify-end">
                                            <Button type="button" variant="danger" size="icon" onClick={() => removeDynamicListItem('story_goals', index)}><Trash2 size={16}/></Button>
                                        </div>
                                    </div>
                                ))}
                                <Button 
                                    type="button" 
                                    variant="outline" 
                                    onClick={() => addDynamicListItem('story_goals')} 
                                    icon={<Plus />}
                                    disabled={product.goal_config !== 'predefined' && product.goal_config !== 'predefined_and_custom'}
                                >
                                    إضافة هدف
                                </Button>
                                {(product.goal_config !== 'predefined' && product.goal_config !== 'predefined_and_custom') && (
                                    <p className="text-xs text-gray-500 mt-2">يجب تفعيل خيار "قائمة محددة" في إعدادات الهدف من القصة لتتمكن من إضافة أهداف.</p>
                                )}
                            </div>
                        </AdminSection>
                    </div>
                    ...
                    <div className="lg:col-span-1">
                        <AdminSection title="الإعدادات" icon={<></>}>
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2 pt-4 border-t">
                                    <label className="flex items-center gap-2"><input type="checkbox" name="is_featured" checked={product.is_featured} onChange={handleSimpleChange}/> منتج مميز</label>
                                    <label className="flex items-center gap-2"><input type="checkbox" name="is_addon" checked={product.is_addon} onChange={handleSimpleChange}/> إضافة إبداعية (Addon)</label>
                                    <label className="flex items-center gap-2"><input type="checkbox" name="has_printed_version" checked={product.has_printed_version} onChange={handleSimpleChange}/> له نسخة مطبوعة</label>
                                </div>
                            </div>
                        </AdminSection>
                    </div>
