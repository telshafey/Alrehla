...
export const TabsList: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className }) => {
    return (
        <div className={cn('inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground mb-6 flex-wrap', className)}>
            {children}
        </div>
    );
};
...
export const TabsContent: React.FC<{ value: string; children: React.ReactNode; className?: string }> = ({ value, children, className }) => {
    ...
    return context.activeTab === value ? <div className={cn('mt-2', className)}>{children}</div> : null;
};
