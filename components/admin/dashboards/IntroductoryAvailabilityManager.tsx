    ...
    return (
        <div className="space-y-6">
            <div className="bg-muted/30 p-4 rounded-xl border border-dashed flex flex-col sm:flex-row gap-4 items-end">
                ...
                <div className="flex-1 w-full">
                    <label className="text-xs font-bold mb-1 block">الوقت (60 دقيقة)</label>
            ...
            <div className="space-y-3">
                {Object.entries(availability).sort().map(([date, times]) => (
                    <div key={date} className="p-3 bg-white border rounded-lg">
                        <p className="font-bold text-sm mb-2 border-b pb-1 text-primary">{date}</p>
                        <div className="flex flex-wrap gap-2">
                            {(times as string[]).map(time => (
                                <span key={time} className="flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-mono font-bold border border-blue-100">
                                    {time}
